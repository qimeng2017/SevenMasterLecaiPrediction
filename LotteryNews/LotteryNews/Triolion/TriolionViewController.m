...
- (void)configUI{
   ...
   // [headerView addSubview:centerView];
    ...
    UIImageView *helpImageview = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(lable.frame), kScreenWidth, 150)];
    helpImageview.image = [UIImage imageNamed:@"1"];
    helpImageview.userInteractionEnabled = YES;
    UITapGestureRecognizer *tapHelp = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapHelpAction)];
    [helpImageview addGestureRecognizer:tapHelp];
    [headerView addSubview:helpImageview];
    headerView.frame = CGRectMake(0, 0, kScreenWidth, CGRectGetHeight(_scrollerView.frame)+CGRectGetHeight(segmentView.frame)+180);
